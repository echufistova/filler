...
/*   Updated: 2018/11/20 20:03:23 by ychufist         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		main(void)
{
	...
	while (get_next_line(0, &line) > 0 && ++game.i < 1)
	{
		game.player = (game.i == 0) ? get_player(line[10]) : game.player;
		game.enemy = (game.player == 'O') ? 'X' : 'O';
		ft_strdel(&line);
	}
	game.field_size = (game.i == 1) ? f_size(line) : game.field_size;
	ft_strdel(&line);
	game.i = 0;
	game.field = (int**)malloc(sizeof(int*) * (game.field_size.y));
	while (get_next_line(0, &line) > 0)
	{
		the_game(&game, &game.i, line, dop);
		ft_strdel(&line);
	}
	free(game.field);
	return (0);
